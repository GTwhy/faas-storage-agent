...
impl Metadata {
    pub fn default() -> Metadata {
        Metadata{
            ...
            endpoint: String::from("redis://127.0.0.1"),
            port: 6379
        }
    }
    pub fn get_url(&self) -> String{
        let url = format!("{}:{}", self.endpoint, self.port.to_string());
        url
    }
}
...
pub fn connect_ns(ns: Namespace) -> ns_resp {
    ...
    let url = md.get_url();
    let _client = redis::Client::open(url).unwrap();
